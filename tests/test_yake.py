...
def test_n1_EN():
    ...
    result = pyake.extract_keywords(text_content)
    print(result)
    res = [('Google', 0.02509259635302287), ('Kaggle', 0.027297150442917317), ('data', 0.07999958986489127), ('science', 0.09834167930168546), ('platform', 0.12404419723925647), ('service', 0.1316357590449064),
           ('acquiring', 0.15110282570329972), ('learning', 0.1620911439042445), ('Goldbloom', 0.1624845364505264), ('machine', 0.16721860165903407), ('competition', 0.1826862004451857), ('Cloud', 0.1849060668345104),
           ('community', 0.202661778267609), ('Ventures', 0.2258881919825325), ('declined', 0.2872980816826787), ('San', 0.2893636939471809), ('Francisco', 0.2893636939471809), ('early', 0.2946076840223411),
...
def test_n1_EL():
    ...
    result = pyake.extract_keywords(text_content)
    print(result)
    res = [('Ουκρανίας', 0.04685829498124156), ('Χάρκοβο', 0.0630891548728466), ('Άμυνας', 0.06395408991254226), ('σύμφωνα', 0.07419311338418161), ('υπουργείου', 0.1069960715371627), ('Ανώτατος', 0.12696931063105557),
           ('διοικητής', 0.18516501832552387), ('ρωσικού', 0.18516501832552387), ('στρατού', 0.18516501832552387), ('φέρεται', 0.18516501832552387), ('σκοτώθηκε', 0.18516501832552387), ('κοντά', 0.18516501832552387),
           ('υπηρεσία', 0.18516501832552387), ('πληροφοριών', 0.18516501832552387), ('Gerasimov', 0.1895400421770795), ('Ρωσίας', 0.1895400421770795), ('Vitaly', 0.24366598777562623), ('Κεντρική', 0.24366598777562623),
...
def test_n1_AR():
    """
    test for the Arabic language
    """
    text_content = '''تستحوذ Google على مجتمع علوم البيانات Kaggle. تخبرنا المصادر أن Google تستحوذ على Kaggle ، وهي منصة تستضيف مسابقات في علوم البيانات والتعلم الآلي. لا تزال التفاصيل حول الصفقة غامضة إلى حد ما ، ولكن بالنظر إلى أن Google تستضيف مؤتمر Cloud Next في سان فرانسيسكو هذا الأسبوع ، فقد يأتي الإعلان الرسمي في وقت مبكر غدًا. عبر الهاتف ، رفض الرئيس التنفيذي المؤسس المشارك لشركة Kaggle ، أنتوني جولدبلوم ، إنكار حدوث عملية الاستحواذ. رفضت جوجل نفسها "التعليق على الشائعات". تم تأسيس Kaggle ، التي تضم حوالي نصف مليون عالم بيانات على منصتها ، بواسطة Goldbloom و Ben Hamner في عام 2010. وقد بدأت الخدمة في وقت مبكر وعلى الرغم من وجود عدد قليل من المنافسين مثل DrivenData و TopCoder و HackerRank ، فقد تمكنت من البقاء أمامهم بوقت طويل من خلال التركيز على مكانته المحددة. تعد الخدمة في الأساس المكان الفعلي لإجراء مسابقات علوم البيانات والتعلم الآلي. مع Kaggle ، تشتري Google واحدًا من أكبر المجتمعات وأكثرها نشاطًا لعلماء البيانات - وبهذا ، ستحصل على مشاركة ذهنية متزايدة في هذا المجتمع أيضًا (على الرغم من أن لديها بالفعل الكثير من ذلك بفضل Tensorflow ومشاريع أخرى). لدى Kaggle القليل من التاريخ مع Google أيضًا ، لكن هذا حديث جدًا. في وقت سابق من هذا الشهر ، تعاونت Google و Kaggle لاستضافة مسابقة للتعلم الآلي بقيمة 100000 دولار حول تصنيف مقاطع فيديو YouTube. كان لتلك المنافسة بعض التكامل العميق مع Google Cloud Platform أيضًا. ما نفهمه هو أن Google ستحافظ على تشغيل الخدمة - على الأرجح تحت اسمها الحالي. في حين أن عملية الاستحواذ ربما تتعلق بمجتمع Kaggle أكثر من كونها تقنية ، إلا أن Kaggle قامت ببناء بعض الأدوات المثيرة للاهتمام لاستضافة منافسيها و "النواة" أيضًا. في Kaggle ، تعد النواة أساسًا الكود المصدري لتحليل مجموعات البيانات ويمكن للمطورين مشاركة هذا الرمز على النظام الأساسي (كانت الشركة قد أطلقت عليهم سابقًا اسم "البرامج النصية"). مثل المواقع المماثلة التي تركز على المنافسة ، يدير Kaggle أيضًا لوحة وظائف أيضًا. من غير الواضح ما الذي ستفعله Google بهذا الجزء من الخدمة. وفقًا لـ Crunchbase ، جمعت Kaggle 12.5 مليون دولار (على الرغم من أن PitchBook تقول إنها 12.75 دولارًا) منذ إطلاقها في عام 2010. يشمل المستثمرون في Kaggle Index Ventures و SV Angel و Max Levchin و Naval Ravikant وكبير الاقتصاديين في Google Hal Varian و Khosla Ventures و Yuri Milner'''

    pyake = yake.KeywordExtractor(lan="ar", n=1, dedupLim=-1)
    result = pyake.extract_keywords(text_content)
    print(result)
    for r in result:
        print(r)
